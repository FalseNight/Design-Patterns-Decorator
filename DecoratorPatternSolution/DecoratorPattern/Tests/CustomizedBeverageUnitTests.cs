...
namespace DecoratorPattern.Tests
{
    ...
    public class CustomizedBeverageUnitTests
    {
        ...
        {
            var drink = new WithWhipCream(WhipCream.Extra, new WithSoyMilk(new WithFlavor(Flavor.Caramel, new Macchiato(Size.Tall))));

            AssertDescriptionContains(drink, "Tall", "Size");
            AssertDescriptionContains(drink, "Soy", "Milk type");
            AssertDescriptionContains(drink, "Macchiato", "Drink Name");
            AssertDescriptionContains(drink, "Caramel", "Flavoring");
